...
const Checkout = () => {
  ...
  const getDeliveries = (item: any) =>
    Array.isArray(item.deliveries)
      ? item.deliveries
      : Array.isArray(item.product_deliveries)
      ? item.product_deliveries
      : item.delivery_type && item.delivery_type !== "none"
      ? [{
          ...
          recipient_mode:
            item.delivery_type === "vip_fac" ? "all_members" : "buyer",
        }]
      : [];

  const needsMemberIds = (item: any) =>
    getDeliveries(item).some(
      (delivery: any) =>
  ...
  const handleCheckout = async () => {
    ...
    for (const item of items.filter((i: any) => needsMemberIds(i))) {
      const ids = (vipRecipients[item.id] || [])
        .map((v) => v.trim())
        .filter(Boolean);
      ...
      if (!ids.length) {
        return alert(`Informe pelo menos um ID para ${item.name}`);
      }

      if (ids.length > max) {
        return alert(`${item.name} permite no máximo ${max} ID(s)`);
      }
      ...
      if (
        ...
        return alert(`${item.name}: remova IDs inválidos ou repetidos`);
      }
    }
        ...
        <div className="w-full max-w-5xl rounded-2xl border border-border bg-card/95 p-6 shadow-2xl">
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
            <div className="space-y-4">
              ...
              {items.filter((item: any) => needsMemberIds(item)).map((item: any) => {
                const max =
                  Math.max(1, Number(item.vip_max_members || 1)) * item.quantity;
                const current = vipRecipients[item.id] || [""];

                const memberRewards = getDeliveries(item).filter(
                  (delivery: any) =>
                    delivery.delivery_type === "vip_fac" ||
                    delivery.recipient_mode === "all_members"
                );
                ...
                return (
                  ...
                  >
                    <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
                      IDs dos membros — {item.name}
                    </h2>

                    <p className="mb-2 text-xs text-muted-foreground">
                      Informe os IDs da cidade que receberão as recompensas configuradas para todos os membros.
                      Limite deste pedido: {max} membro(s).
                    </p>

                    {memberRewards.length > 0 && (
                      <div className="mb-3 rounded-lg border border-border bg-card p-3">
                        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                          Entregas para cada membro
                        </p>
                        <div className="space-y-1 text-xs text-muted-foreground">
                          {memberRewards.map((delivery: any, index: number) => (
                            <p key={`${delivery.delivery_type}-${delivery.delivery_value}-${index}`}>
                              • {delivery.delivery_type}: {delivery.delivery_value}
                              {delivery.delivery_amount > 1
                                ? ` x${delivery.delivery_amount}`
                                : ""}
                              {delivery.delivery_days > 0
                                ? ` — ${delivery.delivery_days} dia(s)`
                                : ""}
                            </p>
                          ))}
                        </div>
                    ...
                    {current.length < max && (
                      <button
                        ...
                        className="mt-3 rounded-md bg-secondary px-4 py-2 text-sm font-semibold"
                      >
                        + Adicionar ID
                      </button>
                    )}
